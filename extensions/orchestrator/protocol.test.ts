...
import { buildOrchestratorProtocol } from "./protocol.js";
...
describe("buildOrchestratorProtocol", () => {
	describe("null and terminal states return null", () => {
		...
		it("returns null for completed state", () => {
			expect(buildOrchestratorProtocol(makeState("completed"))).toBeNull();
		});

		it("returns null for failed state", () => {
			expect(buildOrchestratorProtocol(makeState("failed"))).toBeNull();
		});

		it("returns null for aborted state", () => {
			expect(buildOrchestratorProtocol(makeState("aborted"))).toBeNull();
		});

		it("returns null for idle status", () => {
			const state = makeState("planning");
			// Simulate idle by setting status to a fake value - instead use a cast
			const idleState = { ...state, status: "idle" } as unknown as MissionState;
		...
		it("returns a falsy value for completed (VAL-PROTO-005)", () => {
			expect(buildOrchestratorProtocol(makeState("completed"))).toBeFalsy();
		});

		it("returns a falsy value for failed (VAL-PROTO-005)", () => {
			expect(buildOrchestratorProtocol(makeState("failed"))).toBeFalsy();
		});

		it("returns a falsy value for aborted (VAL-PROTO-005)", () => {
			expect(buildOrchestratorProtocol(makeState("aborted"))).toBeFalsy();
		});
	});

	describe("planning state (VAL-PROTO-001)", () => {
		it("returns a non-null string for planning state", () => {
			const result = buildOrchestratorProtocol(makeState("planning"));
			expect(result).not.toBeNull();
			expect(typeof result).toBe("string");
		});

		it("contains codebase analysis instructions", () => {
			const result = buildOrchestratorProtocol(makeState("planning")) as string;
	...
	describe("draft_review state (VAL-PROTO-002)", () => {
		it("returns a non-null string for draft_review state", () => {
			const result = buildOrchestratorProtocol(makeState("draft_review"));
			expect(result).not.toBeNull();
			expect(typeof result).toBe("string");
		});

		it("instructs waiting for approval", () => {
			const result = buildOrchestratorProtocol(makeState("draft_review")) as string;
	...
	describe("approved state (VAL-PROTO-003)", () => {
		it("returns a non-null string for approved state", () => {
			const result = buildOrchestratorProtocol(makeState("approved"));
			expect(result).not.toBeNull();
			expect(typeof result).toBe("string");
		});

		it("indicates the plan is approved", () => {
			const result = buildOrchestratorProtocol(makeState("approved")) as string;
		...
		it("includes mission description when plan is provided", () => {
			const plan = makePlan({ description: "Build an e-commerce platform" });
			const result = buildOrchestratorProtocol(makeState("approved"), plan) as string;
		...
		it("handles absent plan gracefully", () => {
			const result = buildOrchestratorProtocol(makeState("approved"), undefined);
			expect(result).not.toBeNull();
			expect(typeof result).toBe("string");
		});
	});
	...
	describe("executing state (VAL-PROTO-003)", () => {
		const stateWithFeature = makeState("executing", {
		...
		it("returns a non-null string for executing state", () => {
			const result = buildOrchestratorProtocol(stateWithFeature, makePlan());
			expect(result).not.toBeNull();
			expect(typeof result).toBe("string");
		});

		it("includes current milestone name", () => {
			const result = buildOrchestratorProtocol(stateWithFeature, makePlan()) as string;
		...
		it("includes dirty repo warning when autoCommitEnabled is false", () => {
			...
			const result = buildOrchestratorProtocol(dirtyState, makePlan()) as string;
			expect(result.toLowerCase()).toContain("dirty");
		});

		it("does not include dirty repo warning when autoCommitEnabled is true", () => {
			const cleanState = makeState("executing", {
				currentMilestoneId: "m1",
				currentFeatureId: "f2",
				gitSnapshot: { headCommit: "abc", dirtyFiles: [], autoCommitEnabled: true },
			});
			const result = buildOrchestratorProtocol(cleanState, makePlan()) as string;
	...
	describe("validating state (VAL-PROTO-007)", () => {
		it("returns a non-null string for validating state", () => {
			const result = buildOrchestratorProtocol(makeState("validating"));
			expect(result).not.toBeNull();
			expect(typeof result).toBe("string");
		});

		it("instructs waiting for validation results", () => {
			const result = buildOrchestratorProtocol(makeState("validating")) as string;
		...
		it("is distinct from executing protocol", () => {
			const validating = buildOrchestratorProtocol(makeState("validating")) as string;
			const executing = buildOrchestratorProtocol(
				makeState("executing", { currentMilestoneId: "m1", currentFeatureId: "f2" }),
				makePlan(),
			) as string;
	...
	describe("paused state (VAL-PROTO-004)", () => {
		it("returns a non-null string for paused state", () => {
			const result = buildOrchestratorProtocol(makeState("paused"));
			expect(result).not.toBeNull();
			expect(typeof result).toBe("string");
		});

		it("is concise (approximately ~50-100 tokens, under 500 chars)", () => {
			const result = buildOrchestratorProtocol(makeState("paused")) as string;
	...
	describe("autonomy levels (VAL-CROSS-008)", () => {
		...
		it("three autonomy levels produce distinct content", () => {
			const executingState = makeState("executing", { currentMilestoneId: "m1", currentFeatureId: "f2" });
			const plan = makePlan();
			const low = buildOrchestratorProtocol(executingState, plan, makeConfig({ autonomy: "low" })) as string;
			const medium = buildOrchestratorProtocol(executingState, plan, makeConfig({ autonomy: "medium" })) as string;
			const high = buildOrchestratorProtocol(executingState, plan, makeConfig({ autonomy: "high" })) as string;
	...
	describe("progress summary accuracy (executing state)", () => {
		it("shows correct milestone index (1-based)", () => {
			...
			const result = buildOrchestratorProtocol(state, makePlan()) as string;
	...
	describe("edge cases", () => {
		...
		it("state with no currentMilestoneId in executing does not crash", () => {
			const state = makeState("executing");
			const result = buildOrchestratorProtocol(state, makePlan());
			expect(result).not.toBeNull();
			expect(typeof result).toBe("string");
		});
