...
import setup, { reconcileStateOnStart } from "./index.js";
...
describe("VAL-CROSS-001: full lifecycle", () => {
	...
	it("spawn_worker transitions from approved to executing and updates counters on success", async () => {
		...
		try {
			...
			const { registerSpawnWorkerTool } = await import("./tools/spawn-worker.js");
			const { registerSubmitPlanTool } = await import("./tools/submit-plan.js");
			const { registerUpdateStateTool } = await import("./tools/update-state.js");
			const { registerCompleteMissionTool } = await import("./tools/complete.js");
